# Fiche projet — Équipe 22 

> Livrable L2 · Jalon J1 (samedi 29 août 2026) · validée par l'encadreur référent.
> Aucune fabrication n'est autorisée avant la validation de ce jalon.

## 1. Titre : P022  Détecteur de qualité de l'air en classe

Dispositif D 22 , detecteur de qualite de l'air en classe 

une phrase pour le présenter à un chef d'établissement.

## 2. Besoin et bénéficiaires

 - les Fabmangers et les eleves ont besoin de savoir en permannce la qualité de l'air ainsi que la temperature dans les salles 
 
  - ce dispositif va etre installé dans les Fablab et dans les salles de classe.

## 3. Objectifs d'apprentissage

Trois objectifs observables rattachés au programme officiel, chapitre cité.

1.
2.
3.

## 4. Description du dispositif

Ce que l'objet fait:
- mersure la concentration du dioxyde de carbone dans l'air 
- mesurer la temperature dans la salle 
- afficher les trois nivau d'alerte 
-  fonctionne hors résau
ce que l'élève fait avec :
- exploite les données du dispositif pour prendre les decissions 


 croquis ou esquisse annotée
(versée dans `docs/medias/`).![image du dispositif]. Architecture technique pressentie

Capteurs: 
- Un capteur de dioxyde de carbone à mesure infrarouge non dispersive·
- un capteur de temperature. 

actionneurs:
- les LED 
- le bezzeur 
- l'ecran 

liaison:
- le PCB 
- les cables de connections 

application:
- fusion 360 
- bambu studio
- x-tool 
- vscode 
- mblock s 

 procédés de fabrication envisagés:
 
 - impression 3D, adiditive,  soutractif et traditionnelle 

(au moins trois procédés distincts, exigence ET-FAB-02).

## 6. Rôle des élèves

Position sur le continuum POUR / AVEC / PAR et extension PAR décrite (exigence EP-03).

## 7. Ancrage réseau et implantation

Lab de rattachement 
- CRIT 
-  établissement. 

 lieu d'usage

- Fablab et salle de classe 

 conditions matérielles de la salle:

 - une salle hermétiquement fermé 

## 8. Périmètre

| | Contenu |
|---|---|
| Dans la v1.0 (un boitier foctionnelle ) | |
| En option (Avancé/ Expert) :contrôle de la temperature de aeration et la notification automatique | |
| Explicitement exclu | | 
- la reprogrammation via le resau| 

## 9. Risques et parades

| Risque | Type | Parade |
|---|---|---|
| defaut des capteurs | technique | test et redondance  |
|inssufisance de temps | calendrier | faire des prototype simple  |
|manque de competance | pédagogique | faire de la sous traitence  |

## 10. Budget matière estimé

Grandes masses en FCFA, au regard de la dotation (plafond indicatif : 60 000 FCFA).

## 11. Licences et diffusion

Licences choisies et motivationc:  
- Open source 
 
 accord de l'équipe pour la mise en avant réseau: 
 - poster sur Github public 


## Exemptions demandées

- [ ] ET-FAB-06 (moulage) — justification :
- [ ] ET-MEC-01 (fonction motorisée) — justification :
